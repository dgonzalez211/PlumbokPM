 ...
 * @package Octopush\Plumbok\Compiler\File
...
class TagsUpdater
{
	 ...
	 */
    public function applyNodes(string $filename, Node ...$generated): void {
	 ...
	 */
    private function findNamespace(string $name, Node ...$nodes): ?Node\Stmt\Namespace_ {
	 ...
	 */
    private function findClass(string $name, Node ...$nodes): ?Node\Stmt\Class_ {
     ...
     * @return string
     */
    private function createComment(Node\Stmt\Class_ $originalClass, Node\Stmt\Class_ $generatedClass, Context $context): string {
    ...
    private function createMethodTag(Node\Stmt\ClassMethod $method, Context $context): Method {
        ...
        $arguments = array_map(function (Param $param, int $key) use ($method) {
            ...
            if (
                !empty($default)
                && !($param->getType() instanceof Compound)
